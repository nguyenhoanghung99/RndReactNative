 ...
 * @param suggestion
 */
const getMentionValue = (trigger: string, suggestion: TMembers) =>
  `${trigger}[${suggestion?.fullName}](${suggestion?._id})`;
...
function getUniqueListBy(arr: any, key: string | number) {
  if(arr?.length && arr[0]) {
    return [...new Map(arr.map((item: { [x: string]: any; }) => [item[key], item])).values()] as any[];
  }
  return [];
}
function secondsToHms(d: number) {
  ...
  const hDisplay = h > 0 ? h > 9 ? h.toString() : `0${h}:` : '00:'
  const mDisplay = m > 0 ? m > 9 ? m.toString()  : `0${m}:` : '00:'
  const sDisplay = s > 0 ? s > 9 ? s.toString()  : `0${s}` : '00'
